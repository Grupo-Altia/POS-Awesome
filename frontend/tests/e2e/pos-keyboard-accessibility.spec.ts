...
test.skip(!ENABLED, "Set POSA_KEYBOARD_E2E=1 to run real POS keyboard E2E tests.");
...
async function loginIfCredentialsProvided(page: Page) {
	...
	const loginButton = page
		...
		.or(page.locator('button:has-text("Login"), button:has-text("Log In")'));

	if (!(await userInput.count()) || !(await passInput.count())) return;
...
async function callFrappe<T = any>(page: Page, method: string, args: Record<string, unknown> = {}) {
...
async function getPositiveStockItems(page: Page): Promise<TestItem[]> {
	const rows = await page.evaluate(
		...
		{ codes: TEST_ITEM_CODES },
	);

	const items: TestItem[] = [];
	for (const item of rows as Array<{ item_code: string; item_name: string }>) {
		const [stockRows, priceRows] = await Promise.all([
			callFrappe<Array<{ actual_qty: number }>>(page, "frappe.client.get_list", {
				doctype: "Bin",
				filters: { item_code: item.item_code },
				fields: ["actual_qty"],
				limit_page_length: 20,
			}),
			callFrappe<Array<{ price_list_rate: number }>>(page, "frappe.client.get_list", {
				doctype: "Item Price",
		...
		const stock = (stockRows || []).reduce((total, row) => total + Number(row.actual_qty || 0), 0);
		const rate = Number(priceRows?.[0]?.price_list_rate || 0);
		if (stock > 0 && rate > 0) {
...
async function enterInvoiceGrid(page: Page) {
	...
	await expect(page.locator(".posa-cart-item-row--keyboard-active").first()).toBeVisible({
		timeout: 10000,
	});
}
...
async function expectNoActiveInvoiceGrid(page: Page) {
	await expect(page.locator(".posa-cart-item-row--keyboard-active")).toHaveCount(0);
	await expect(page.locator(".posa-cart-item-cell--keyboard-active")).toHaveCount(0);
}

async function openInvoiceManagement(page: Page) {
	await page.getByTestId("invoice-action-management").click();
	await expect(page.getByTestId("invoice-management-dialog")).toBeVisible({ timeout: 30000 });
}

async function submitCurrentInvoice(page: Page) {
	await page.getByTestId("invoice-action-pay").click();
	await expect(page.getByTestId("payment-root")).toBeVisible({ timeout: 30000 });
	await expect(page.locator("[data-pos-keyboard-target='payment-amount'] input").first()).toBeFocused({
		timeout: 15000,
	});
...
async function latestSubmittedInvoice(page: Page) {
	const mode = await getValue<{ create_pos_invoice_instead_of_sales_invoice?: number }>(
		page,
		"POS Profile",
		POS_PROFILE,
		"create_pos_invoice_instead_of_sales_invoice",
	);
	const doctype = Number(mode?.create_pos_invoice_instead_of_sales_invoice || 0)
		? "POS Invoice"
		: "Sales Invoice";
	...
	return { doctype, name: rows[0].name, grand_total: Number(rows[0].grand_total || 0) };
}
...
async function openSubmittedInvoiceEdit(page: Page, invoiceName: string) {
	...
	await expect(page.getByTestId(`invoice-management-edit-${invoiceName}`)).toBeVisible({
		timeout: 30000,
	});
	await page.getByTestId(`invoice-management-edit-${invoiceName}`).click();
	await expect(page.getByTestId("invoice-edit-modal")).toBeVisible({ timeout: 30000 });
}
...
test.describe.serial("POS keyboard accessibility E2E", () => {
	...
	test.beforeAll(async ({ browser }) => {
		...
		originalQuickEditFlag = Number(profile?.posa_allow_item_quick_edit || 0);
		await setValue(page, "POS Profile", POS_PROFILE, "posa_allow_item_quick_edit", 1);
		await page.close();
	});
	...
	test("product search down arrow stays in product results and does not steal focus to cart", async ({
		...
		expect(items.length, `Need at least 3 positive-stock items from ${TEST_ITEM_CODES.join(", ")}`).toBeGreaterThanOrEqual(3);
	...
	test("operator can add items and edit invoice grid fully from keyboard", async ({ page }) => {
		...
		await expect(await activeCartCell(page)).toBeVisible();
		await setActiveNumericCell(page, String(Math.max(1, Math.round(items[0].rate))));

		await page.keyboard.press("Escape");
		await expect(page.locator(".posa-cart-item-row--keyboard-active").first()).toBeVisible();
		await page.keyboard.press("Enter");
		await expect(page.getByTestId("item-history-modal")).toBeVisible({ timeout: 30000 });
		await expect(page.locator(".posa-modal-keyboard-box").first()).toBeVisible();
		await page.keyboard.press("ArrowRight");
		await page.keyboard.press("Enter");
		await expect(page.getByTestId("item-history-details-tab")).toHaveAttribute("aria-selected", "true");
		await page.keyboard.press("Escape");
		await expect(page.getByTestId("item-history-modal")).toBeHidden({ timeout: 15000 });
	});

	test("item quick edit opens from selected cart row and is keyboard reachable", async ({ page }) => {
		if (!items.length) items = await getPositiveStockItems(page);
		await searchAndAddItem(page, items[0]);
		await enterInvoiceGrid(page);
		await page.keyboard.press("F12");

		await expect(page.getByTestId("item-quick-edit-modal")).toBeVisible({ timeout: 30000 });
		await expect(page.getByTestId("item-quick-edit-lookup").locator("input")).toHaveValue(items[0].item_code, {
			timeout: 30000,
		});
		await tabUntilFocused(page, page.getByTestId("item-quick-edit-name").locator("input"));
		await tabUntilFocused(page, page.getByTestId("item-quick-edit-short-name").locator("input"));
		await page.keyboard.press("Escape");
		await expect(page.getByTestId("item-quick-edit-modal")).toBeHidden({ timeout: 15000 });
	});

	test("drafts drawer supports keyboard load and escape paths", async ({ page }) => {
		if (!items.length) items = await getPositiveStockItems(page);
		await searchAndAddItem(page, items[0]);
	...
	test("invoice management and submitted invoice edit modal are keyboard operable", async ({ page }) => {
		...
		await expect(page.locator(".edit-keyboard-box").first()).toBeVisible({ timeout: 15000 });
		await page.keyboard.press("ArrowDown");
		await page.keyboard.press("Enter");
		...
		const qtyInput = page.getByTestId("invoice-edit-item-0-qty").locator("input");
		await qtyInput.fill("3");
		await expect(page.getByTestId("invoice-edit-settlement-summary")).toContainText(
			/Collect from customer|Refund to customer|No cash difference/,
			{ timeout: 30000 },
		);
		const paymentInput = page.getByTestId("invoice-edit-payment-0-amount").locator("input");
		await expect(paymentInput).toHaveAttribute("readonly", /readonly|/);

		await page.keyboard.press("Escape");
		await expect(page.getByTestId("invoice-edit-modal")).toBeHidden({ timeout: 15000 });
	});
});
